...
def test_push_file_empty_base_path(storage):
...
def test_push_file_nonempty_base_path(storage):
    base_path = "base_path"
    storage.set_remote_base_path(base_path)
    storage.delete("sample.txt")
    remote_objects = storage.push("sample.txt")
...
def test_push_directory(storage):
...
def test_pull_file(storage, tmpdir):
...
def test_pull_dir(storage, tmpdir):
    local_base_dir = tmpdir.mkdir("remote_storage")
